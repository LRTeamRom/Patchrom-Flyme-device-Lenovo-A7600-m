.class public final Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;
.super Lcom/flyme/deviceoriginalsettings/bluetooth/DeviceListPreferenceFragment;
.source "BluetoothSettings.java"

# interfaces
.implements Lcom/lenovo/settings/search/Indexable;


# static fields
.field private static final BTOPP_ACTION_OPEN_RECEIVED_FILES:Ljava/lang/String; = "android.btopp.intent.action.OPEN_RECEIVED_FILES"

.field private static final LAUNCH_BT_ADVANCED_SETTINGS_ACTION:Ljava/lang/String; = "com.mediatek.bluetooth.settings.action.START_BT_ADV_SETTING"

.field private static final MENU_ID_ADVANCED_SETTING:I = 0x5

.field private static final MENU_ID_RENAME_DEVICE:I = 0x2

.field private static final MENU_ID_SCAN:I = 0x1

.field private static final MENU_ID_SHOW_RECEIVED:I = 0x4

.field private static final MENU_ID_VISIBILITY_TIMEOUT:I = 0x3

.field public static final SEARCH_INDEX_DATA_PROVIDER:Lcom/lenovo/settings/search/Indexable$SearchIndexProvider;

.field private static final TAG:Ljava/lang/String; = "BluetoothSettings"


# instance fields
.field private mActivityStarted:Z

.field private mAvailableDevicesCategory:Landroid/preference/PreferenceGroup;

.field private mAvailableDevicesCategoryIsPresent:Z

.field private mBluetoothEnabler:Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothEnabler;

.field private final mDeviceProfilesListener:Landroid/view/View$OnClickListener;

.field private mDiscoverableEnabler:Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDiscoverableEnabler;

.field private mEmptyView:Landroid/widget/TextView;

.field private final mIntentFilter:Landroid/content/IntentFilter;

.field mMyDevicePreference:Landroid/preference/Preference;

.field private mPairedDevicesCategory:Landroid/preference/PreferenceGroup;

.field private final mReceiver:Landroid/content/BroadcastReceiver;


# direct methods
.method static constructor <clinit>()V
    .locals 1

    .prologue
    .line 500
    new-instance v0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings$4;

    invoke-direct {v0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings$4;-><init>()V

    sput-object v0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->SEARCH_INDEX_DATA_PROVIDER:Lcom/lenovo/settings/search/Indexable$SearchIndexProvider;

    return-void
.end method

.method public constructor <init>()V
    .locals 2

    .prologue
    .line 123
    const-string v0, "no_config_bluetooth"

    invoke-direct {p0, v0}, Lcom/flyme/deviceoriginalsettings/bluetooth/DeviceListPreferenceFragment;-><init>(Ljava/lang/String;)V

    .line 106
    new-instance v0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings$1;

    invoke-direct {v0, p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings$1;-><init>(Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;)V

    iput-object v0, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mReceiver:Landroid/content/BroadcastReceiver;

    .line 459
    new-instance v0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings$3;

    invoke-direct {v0, p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings$3;-><init>(Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;)V

    iput-object v0, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mDeviceProfilesListener:Landroid/view/View$OnClickListener;

    .line 124
    new-instance v0, Landroid/content/IntentFilter;

    const-string v1, "android.bluetooth.adapter.action.LOCAL_NAME_CHANGED"

    invoke-direct {v0, v1}, Landroid/content/IntentFilter;-><init>(Ljava/lang/String;)V

    iput-object v0, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mIntentFilter:Landroid/content/IntentFilter;

    .line 125
    return-void
.end method

.method static synthetic access$000(Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;)Z
    .locals 1
    .param p0, "x0"    # Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;

    .prologue
    .line 70
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->isRestrictedAndNotPinProtected()Z

    move-result v0

    return v0
.end method

.method private addDeviceCategory(Landroid/preference/PreferenceGroup;ILcom/lenovoframework/adapter/essential/Filter;)V
    .locals 1
    .param p1, "preferenceGroup"    # Landroid/preference/PreferenceGroup;
    .param p2, "titleId"    # I
    .param p3, "filter"    # Lcom/lenovoframework/adapter/essential/Filter;

    .prologue
    .line 331
    invoke-virtual {p1, p2}, Landroid/preference/PreferenceGroup;->setTitle(I)V

    .line 332
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getPreferenceScreen()Landroid/preference/PreferenceScreen;

    move-result-object v0

    invoke-virtual {v0, p1}, Landroid/preference/PreferenceScreen;->addPreference(Landroid/preference/Preference;)Z

    .line 333
    invoke-virtual {p0, p3}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->setFilter(Lcom/lenovoframework/adapter/essential/Filter;)V

    .line 334
    invoke-virtual {p0, p1}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->setDeviceListGroup(Landroid/preference/PreferenceGroup;)V

    .line 335
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->addCachedDevices()V

    .line 336
    const/4 v0, 0x1

    invoke-virtual {p1, v0}, Landroid/preference/PreferenceGroup;->setEnabled(Z)V

    .line 337
    return-void
.end method

.method private startScanning()V
    .locals 2

    .prologue
    .line 313
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->isRestrictedAndNotPinProtected()Z

    move-result v0

    if-eqz v0, :cond_0

    .line 318
    :goto_0
    return-void

    .line 314
    :cond_0
    iget-boolean v0, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mAvailableDevicesCategoryIsPresent:Z

    if-nez v0, :cond_1

    .line 315
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getPreferenceScreen()Landroid/preference/PreferenceScreen;

    move-result-object v0

    iget-object v1, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mAvailableDevicesCategory:Landroid/preference/PreferenceGroup;

    invoke-virtual {v0, v1}, Landroid/preference/PreferenceScreen;->addPreference(Landroid/preference/Preference;)Z

    .line 317
    :cond_1
    iget-object v0, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mLocalAdapter:Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;

    const/4 v1, 0x1

    invoke-virtual {v0, v1}, Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;->startScanning(Z)V

    goto :goto_0
.end method

.method private updateContent(IZ)V
    .locals 10
    .param p1, "bluetoothState"    # I
    .param p2, "scanState"    # Z

    .prologue
    const/4 v9, 0x0

    const/4 v8, 0x1

    .line 340
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getPreferenceScreen()Landroid/preference/PreferenceScreen;

    move-result-object v3

    .line 341
    .local v3, "preferenceScreen":Landroid/preference/PreferenceScreen;
    const/4 v0, 0x0

    .line 343
    .local v0, "messageId":I
    packed-switch p1, :pswitch_data_0

    .line 434
    :goto_0
    invoke-virtual {p0, v3}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->setDeviceListGroup(Landroid/preference/PreferenceGroup;)V

    .line 435
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->removeAllDevices()V

    .line 436
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mEmptyView:Landroid/widget/TextView;

    invoke-virtual {v4, v0}, Landroid/widget/TextView;->setText(I)V

    .line 437
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getActivity()Landroid/app/Activity;

    move-result-object v4

    invoke-virtual {v4}, Landroid/app/Activity;->invalidateOptionsMenu()V

    .line 438
    :goto_1
    return-void

    .line 345
    :pswitch_0
    invoke-virtual {v3}, Landroid/preference/PreferenceScreen;->removeAll()V

    .line 346
    invoke-virtual {v3, v8}, Landroid/preference/PreferenceScreen;->setOrderingAsAdded(Z)V

    .line 347
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mDevicePreferenceMap:Ljava/util/WeakHashMap;

    invoke-virtual {v4}, Ljava/util/WeakHashMap;->clear()V

    .line 350
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mMyDevicePreference:Landroid/preference/Preference;

    if-nez v4, :cond_0

    .line 351
    new-instance v4, Landroid/preference/Preference;

    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getActivity()Landroid/app/Activity;

    move-result-object v5

    invoke-direct {v4, v5}, Landroid/preference/Preference;-><init>(Landroid/content/Context;)V

    iput-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mMyDevicePreference:Landroid/preference/Preference;

    .line 353
    :cond_0
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mMyDevicePreference:Landroid/preference/Preference;

    iget-object v5, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mLocalAdapter:Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;

    invoke-virtual {v5}, Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;->getName()Ljava/lang/String;

    move-result-object v5

    invoke-virtual {v4, v5}, Landroid/preference/Preference;->setTitle(Ljava/lang/CharSequence;)V

    .line 354
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getResources()Landroid/content/res/Resources;

    move-result-object v4

    const v5, 0x1120043

    invoke-virtual {v4, v5}, Landroid/content/res/Resources;->getBoolean(I)Z

    move-result v4

    if-eqz v4, :cond_6

    .line 355
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mMyDevicePreference:Landroid/preference/Preference;

    const v5, 0x7f020077

    invoke-virtual {v4, v5}, Landroid/preference/Preference;->setIcon(I)V

    .line 359
    :goto_2
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mMyDevicePreference:Landroid/preference/Preference;

    invoke-virtual {v4, v9}, Landroid/preference/Preference;->setPersistent(Z)V

    .line 360
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mMyDevicePreference:Landroid/preference/Preference;

    invoke-virtual {v4, v8}, Landroid/preference/Preference;->setEnabled(Z)V

    .line 361
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mMyDevicePreference:Landroid/preference/Preference;

    invoke-virtual {v3, v4}, Landroid/preference/PreferenceScreen;->addPreference(Landroid/preference/Preference;)Z

    .line 363
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->isRestrictedAndNotPinProtected()Z

    move-result v4

    if-nez v4, :cond_1

    .line 364
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mDiscoverableEnabler:Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDiscoverableEnabler;

    if-nez v4, :cond_1

    .line 365
    new-instance v4, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDiscoverableEnabler;

    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getActivity()Landroid/app/Activity;

    move-result-object v5

    iget-object v6, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mLocalAdapter:Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;

    iget-object v7, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mMyDevicePreference:Landroid/preference/Preference;

    invoke-direct {v4, v5, v6, v7}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDiscoverableEnabler;-><init>(Landroid/content/Context;Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;Landroid/preference/Preference;)V

    iput-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mDiscoverableEnabler:Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDiscoverableEnabler;

    .line 367
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mDiscoverableEnabler:Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDiscoverableEnabler;

    invoke-virtual {v4}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDiscoverableEnabler;->resume()V

    .line 368
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getActivity()Landroid/app/Activity;

    move-result-object v4

    invoke-static {v4}, Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothManager;->getInstance(Landroid/content/Context;)Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothManager;

    move-result-object v4

    iget-object v5, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mDiscoverableEnabler:Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDiscoverableEnabler;

    invoke-virtual {v4, v5}, Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothManager;->setDiscoverableEnabler(Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDiscoverableEnabler;)V

    .line 374
    :cond_1
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mPairedDevicesCategory:Landroid/preference/PreferenceGroup;

    if-nez v4, :cond_7

    .line 375
    new-instance v4, Landroid/preference/PreferenceCategory;

    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getActivity()Landroid/app/Activity;

    move-result-object v5

    invoke-direct {v4, v5}, Landroid/preference/PreferenceCategory;-><init>(Landroid/content/Context;)V

    iput-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mPairedDevicesCategory:Landroid/preference/PreferenceGroup;

    .line 379
    :goto_3
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mPairedDevicesCategory:Landroid/preference/PreferenceGroup;

    const v5, 0x7f0a068d

    sget-object v6, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDeviceFilter;->BONDED_DEVICE_FILTER:Lcom/lenovoframework/adapter/essential/Filter;

    invoke-direct {p0, v4, v5, v6}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->addDeviceCategory(Landroid/preference/PreferenceGroup;ILcom/lenovoframework/adapter/essential/Filter;)V

    .line 382
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mPairedDevicesCategory:Landroid/preference/PreferenceGroup;

    invoke-virtual {v4}, Landroid/preference/PreferenceGroup;->getPreferenceCount()I

    move-result v2

    .line 384
    .local v2, "numberOfPairedDevices":I
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mDiscoverableEnabler:Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDiscoverableEnabler;

    if-eqz v4, :cond_2

    .line 385
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mDiscoverableEnabler:Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDiscoverableEnabler;

    invoke-virtual {v4, v2}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDiscoverableEnabler;->setNumberOfPairedDevices(I)V

    .line 389
    :cond_2
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mAvailableDevicesCategory:Landroid/preference/PreferenceGroup;

    if-nez v4, :cond_8

    .line 390
    new-instance v4, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothProgressCategory;

    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getActivity()Landroid/app/Activity;

    move-result-object v5

    const/4 v6, 0x0

    invoke-direct {v4, v5, v6}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothProgressCategory;-><init>(Landroid/content/Context;Landroid/util/AttributeSet;)V

    iput-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mAvailableDevicesCategory:Landroid/preference/PreferenceGroup;

    .line 394
    :goto_4
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->isRestrictedAndNotPinProtected()Z

    move-result v4

    if-nez v4, :cond_3

    .line 395
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mAvailableDevicesCategory:Landroid/preference/PreferenceGroup;

    const v5, 0x7f0a068e

    sget-object v6, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDeviceFilter;->UNBONDED_DEVICE_FILTER:Lcom/lenovoframework/adapter/essential/Filter;

    invoke-direct {p0, v4, v5, v6}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->addDeviceCategory(Landroid/preference/PreferenceGroup;ILcom/lenovoframework/adapter/essential/Filter;)V

    .line 399
    :cond_3
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mAvailableDevicesCategory:Landroid/preference/PreferenceGroup;

    invoke-virtual {v4}, Landroid/preference/PreferenceGroup;->getPreferenceCount()I

    move-result v1

    .line 400
    .local v1, "numberOfAvailableDevices":I
    iput-boolean v8, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mAvailableDevicesCategoryIsPresent:Z

    .line 402
    if-nez v1, :cond_4

    .line 403
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mAvailableDevicesCategory:Landroid/preference/PreferenceGroup;

    invoke-virtual {v3, v4}, Landroid/preference/PreferenceScreen;->removePreference(Landroid/preference/Preference;)Z

    .line 404
    iput-boolean v9, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mAvailableDevicesCategoryIsPresent:Z

    .line 407
    :cond_4
    if-nez v2, :cond_5

    .line 408
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mPairedDevicesCategory:Landroid/preference/PreferenceGroup;

    invoke-virtual {v3, v4}, Landroid/preference/PreferenceScreen;->removePreference(Landroid/preference/Preference;)Z

    .line 409
    if-ne p2, v8, :cond_9

    .line 410
    iput-boolean v9, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mActivityStarted:Z

    .line 411
    invoke-direct {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->startScanning()V

    .line 418
    :cond_5
    :goto_5
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getActivity()Landroid/app/Activity;

    move-result-object v4

    invoke-virtual {v4}, Landroid/app/Activity;->invalidateOptionsMenu()V

    goto/16 :goto_1

    .line 357
    .end local v1    # "numberOfAvailableDevices":I
    .end local v2    # "numberOfPairedDevices":I
    :cond_6
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mMyDevicePreference:Landroid/preference/Preference;

    const v5, 0x7f02007d

    invoke-virtual {v4, v5}, Landroid/preference/Preference;->setIcon(I)V

    goto/16 :goto_2

    .line 377
    :cond_7
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mPairedDevicesCategory:Landroid/preference/PreferenceGroup;

    invoke-virtual {v4}, Landroid/preference/PreferenceGroup;->removeAll()V

    goto :goto_3

    .line 392
    .restart local v2    # "numberOfPairedDevices":I
    :cond_8
    iget-object v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mAvailableDevicesCategory:Landroid/preference/PreferenceGroup;

    invoke-virtual {v4}, Landroid/preference/PreferenceGroup;->removeAll()V

    goto :goto_4

    .line 413
    .restart local v1    # "numberOfAvailableDevices":I
    :cond_9
    iget-boolean v4, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mAvailableDevicesCategoryIsPresent:Z

    if-nez v4, :cond_5

    .line 414
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getPreferenceScreen()Landroid/preference/PreferenceScreen;

    move-result-object v4

    iget-object v5, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mAvailableDevicesCategory:Landroid/preference/PreferenceGroup;

    invoke-virtual {v4, v5}, Landroid/preference/PreferenceScreen;->addPreference(Landroid/preference/Preference;)Z

    goto :goto_5

    .line 422
    .end local v1    # "numberOfAvailableDevices":I
    .end local v2    # "numberOfPairedDevices":I
    :pswitch_1
    const v0, 0x7f0a0596

    .line 423
    goto/16 :goto_0

    .line 426
    :pswitch_2
    const v0, 0x7f0a0697

    .line 427
    goto/16 :goto_0

    .line 430
    :pswitch_3
    const v0, 0x7f0a0595

    goto/16 :goto_0

    .line 343
    :pswitch_data_0
    .packed-switch 0xa
        :pswitch_2
        :pswitch_3
        :pswitch_0
        :pswitch_1
    .end packed-switch
.end method


# virtual methods
.method addPreferencesForActivity()V
    .locals 8

    .prologue
    const/16 v7, 0x10

    const/4 v6, -0x2

    const/4 v5, 0x0

    .line 138
    const v3, 0x7f05000c

    invoke-virtual {p0, v3}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->addPreferencesFromResource(I)V

    .line 140
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getActivity()Landroid/app/Activity;

    move-result-object v1

    .line 142
    .local v1, "activity":Landroid/app/Activity;
    new-instance v0, Landroid/widget/Switch;

    invoke-direct {v0, v1}, Landroid/widget/Switch;-><init>(Landroid/content/Context;)V

    .line 144
    .local v0, "actionBarSwitch":Landroid/widget/Switch;
    invoke-virtual {v1}, Landroid/app/Activity;->getResources()Landroid/content/res/Resources;

    move-result-object v3

    const v4, 0x7f0d0002

    invoke-virtual {v3, v4}, Landroid/content/res/Resources;->getDimensionPixelSize(I)I

    move-result v2

    .line 146
    .local v2, "padding":I
    invoke-virtual {v0, v5, v5, v2, v5}, Landroid/widget/Switch;->setPaddingRelative(IIII)V

    .line 147
    invoke-virtual {v1}, Landroid/app/Activity;->getActionBar()Landroid/app/ActionBar;

    move-result-object v3

    invoke-virtual {v3, v7, v7}, Landroid/app/ActionBar;->setDisplayOptions(II)V

    .line 149
    invoke-virtual {v1}, Landroid/app/Activity;->getActionBar()Landroid/app/ActionBar;

    move-result-object v3

    new-instance v4, Landroid/app/ActionBar$LayoutParams;

    const v5, 0x800015

    invoke-direct {v4, v6, v6, v5}, Landroid/app/ActionBar$LayoutParams;-><init>(III)V

    invoke-virtual {v3, v0, v4}, Landroid/app/ActionBar;->setCustomView(Landroid/view/View;Landroid/app/ActionBar$LayoutParams;)V

    .line 153
    new-instance v3, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothEnabler;

    invoke-direct {v3, v1, v0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothEnabler;-><init>(Landroid/content/Context;Landroid/widget/Switch;)V

    iput-object v3, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mBluetoothEnabler:Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothEnabler;

    .line 155
    new-instance v3, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings$2;

    invoke-direct {v3, p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings$2;-><init>(Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;)V

    invoke-virtual {v0, v3}, Landroid/widget/Switch;->setOnClickListener(Landroid/view/View$OnClickListener;)V

    .line 170
    const/4 v3, 0x1

    invoke-virtual {p0, v3}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->setHasOptionsMenu(Z)V

    .line 171
    return-void
.end method

.method initDevicePreference(Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDevicePreference;)V
    .locals 3
    .param p1, "preference"    # Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDevicePreference;

    .prologue
    .line 485
    invoke-virtual {p1}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDevicePreference;->getCachedDevice()Lcom/flyme/deviceoriginalsettings/bluetooth/CachedBluetoothDevice;

    move-result-object v0

    .line 486
    .local v0, "cachedDevice":Lcom/flyme/deviceoriginalsettings/bluetooth/CachedBluetoothDevice;
    invoke-virtual {v0}, Lcom/flyme/deviceoriginalsettings/bluetooth/CachedBluetoothDevice;->getBondState()I

    move-result v1

    const/16 v2, 0xc

    if-ne v1, v2, :cond_0

    .line 488
    iget-object v1, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mDeviceProfilesListener:Landroid/view/View$OnClickListener;

    invoke-virtual {p1, v1}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDevicePreference;->setOnSettingsClickListener(Landroid/view/View$OnClickListener;)V

    .line 490
    :cond_0
    return-void
.end method

.method public onActivityCreated(Landroid/os/Bundle;)V
    .locals 2
    .param p1, "savedInstanceState"    # Landroid/os/Bundle;

    .prologue
    .line 129
    invoke-super {p0, p1}, Lcom/flyme/deviceoriginalsettings/bluetooth/DeviceListPreferenceFragment;->onActivityCreated(Landroid/os/Bundle;)V

    .line 130
    if-nez p1, :cond_0

    const/4 v0, 0x1

    :goto_0
    iput-boolean v0, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mActivityStarted:Z

    .line 132
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getView()Landroid/view/View;

    move-result-object v0

    const v1, 0x1020004

    invoke-virtual {v0, v1}, Landroid/view/View;->findViewById(I)Landroid/view/View;

    move-result-object v0

    check-cast v0, Landroid/widget/TextView;

    iput-object v0, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mEmptyView:Landroid/widget/TextView;

    .line 133
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getListView()Landroid/widget/ListView;

    move-result-object v0

    iget-object v1, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mEmptyView:Landroid/widget/TextView;

    invoke-virtual {v0, v1}, Landroid/widget/ListView;->setEmptyView(Landroid/view/View;)V

    .line 134
    return-void

    .line 130
    :cond_0
    const/4 v0, 0x0

    goto :goto_0
.end method

.method public onBluetoothStateChanged(I)V
    .locals 1
    .param p1, "bluetoothState"    # I

    .prologue
    .line 442
    invoke-super {p0, p1}, Lcom/flyme/deviceoriginalsettings/bluetooth/DeviceListPreferenceFragment;->onBluetoothStateChanged(I)V

    .line 443
    const/4 v0, 0x1

    invoke-direct {p0, p1, v0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->updateContent(IZ)V

    .line 444
    return-void
.end method

.method public onCreateOptionsMenu(Landroid/view/Menu;Landroid/view/MenuInflater;)V
    .locals 7
    .param p1, "menu"    # Landroid/view/Menu;
    .param p2, "inflater"    # Landroid/view/MenuInflater;

    .prologue
    const/4 v3, 0x1

    const/4 v4, 0x0

    .line 213
    iget-object v5, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mLocalAdapter:Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;

    if-nez v5, :cond_1

    .line 252
    :cond_0
    :goto_0
    return-void

    .line 215
    :cond_1
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->isRestrictedAndNotPinProtected()Z

    move-result v5

    if-nez v5, :cond_0

    .line 217
    iget-object v5, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mLocalAdapter:Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;

    invoke-virtual {v5}, Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;->getBluetoothState()I

    move-result v5

    const/16 v6, 0xc

    if-ne v5, v6, :cond_2

    move v0, v3

    .line 218
    .local v0, "bluetoothIsEnabled":Z
    :goto_1
    iget-object v5, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mLocalAdapter:Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;

    invoke-virtual {v5}, Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;->isDiscovering()Z

    move-result v1

    .line 226
    .local v1, "isDiscovering":Z
    if-eqz v1, :cond_3

    const v2, 0x7f0a0275

    .line 227
    .local v2, "textId":I
    :goto_2
    invoke-interface {p1, v4, v3, v4, v2}, Landroid/view/Menu;->add(IIII)Landroid/view/MenuItem;

    move-result-object v3

    invoke-interface {v3, v0}, Landroid/view/MenuItem;->setEnabled(Z)Landroid/view/MenuItem;

    move-result-object v3

    invoke-interface {v3, v4}, Landroid/view/MenuItem;->setShowAsAction(I)V

    .line 231
    const/4 v3, 0x2

    const v5, 0x7f0a0577

    invoke-interface {p1, v4, v3, v4, v5}, Landroid/view/Menu;->add(IIII)Landroid/view/MenuItem;

    move-result-object v3

    invoke-interface {v3, v0}, Landroid/view/MenuItem;->setEnabled(Z)Landroid/view/MenuItem;

    move-result-object v3

    invoke-interface {v3, v4}, Landroid/view/MenuItem;->setShowAsAction(I)V

    .line 234
    const/4 v3, 0x3

    const v5, 0x7f0a056e

    invoke-interface {p1, v4, v3, v4, v5}, Landroid/view/Menu;->add(IIII)Landroid/view/MenuItem;

    move-result-object v3

    invoke-interface {v3, v0}, Landroid/view/MenuItem;->setEnabled(Z)Landroid/view/MenuItem;

    move-result-object v3

    invoke-interface {v3, v4}, Landroid/view/MenuItem;->setShowAsAction(I)V

    .line 237
    const/4 v3, 0x4

    const v5, 0x7f0a058d

    invoke-interface {p1, v4, v3, v4, v5}, Landroid/view/Menu;->add(IIII)Landroid/view/MenuItem;

    move-result-object v3

    invoke-interface {v3, v4}, Landroid/view/MenuItem;->setShowAsAction(I)V

    .line 251
    invoke-super {p0, p1, p2}, Lcom/flyme/deviceoriginalsettings/bluetooth/DeviceListPreferenceFragment;->onCreateOptionsMenu(Landroid/view/Menu;Landroid/view/MenuInflater;)V

    goto :goto_0

    .end local v0    # "bluetoothIsEnabled":Z
    .end local v1    # "isDiscovering":Z
    .end local v2    # "textId":I
    :cond_2
    move v0, v4

    .line 217
    goto :goto_1

    .line 226
    .restart local v0    # "bluetoothIsEnabled":Z
    .restart local v1    # "isDiscovering":Z
    :cond_3
    const v2, 0x7f0a068a

    goto :goto_2
.end method

.method public onDeviceBondStateChanged(Lcom/flyme/deviceoriginalsettings/bluetooth/CachedBluetoothDevice;I)V
    .locals 2
    .param p1, "cachedDevice"    # Lcom/flyme/deviceoriginalsettings/bluetooth/CachedBluetoothDevice;
    .param p2, "bondState"    # I

    .prologue
    .line 454
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getPreferenceScreen()Landroid/preference/PreferenceScreen;

    move-result-object v0

    invoke-virtual {p0, v0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->setDeviceListGroup(Landroid/preference/PreferenceGroup;)V

    .line 455
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->removeAllDevices()V

    .line 456
    iget-object v0, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mLocalAdapter:Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;

    invoke-virtual {v0}, Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;->getBluetoothState()I

    move-result v0

    const/4 v1, 0x0

    invoke-direct {p0, v0, v1}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->updateContent(IZ)V

    .line 457
    return-void
.end method

.method onDevicePreferenceClick(Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDevicePreference;)V
    .locals 1
    .param p1, "btPreference"    # Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDevicePreference;

    .prologue
    .line 322
    iget-object v0, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mLocalAdapter:Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;

    invoke-virtual {v0}, Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;->stopScanning()V

    .line 323
    invoke-super {p0, p1}, Lcom/flyme/deviceoriginalsettings/bluetooth/DeviceListPreferenceFragment;->onDevicePreferenceClick(Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDevicePreference;)V

    .line 324
    return-void
.end method

.method public onOptionsItemSelected(Landroid/view/MenuItem;)Z
    .locals 12
    .param p1, "item"    # Landroid/view/MenuItem;

    .prologue
    const v1, 0x7f0a0676

    const/4 v11, 0x1

    .line 257
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getActivity()Landroid/app/Activity;

    move-result-object v0

    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getActivity()Landroid/app/Activity;

    move-result-object v2

    invoke-virtual {v2}, Landroid/app/Activity;->getPackageName()Ljava/lang/String;

    move-result-object v2

    invoke-virtual {p0}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    move-result-object v3

    invoke-virtual {v3}, Ljava/lang/Class;->getName()Ljava/lang/String;

    move-result-object v4

    const/4 v5, 0x0

    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getActivity()Landroid/app/Activity;

    move-result-object v3

    invoke-virtual {v3}, Landroid/app/Activity;->getPackageName()Ljava/lang/String;

    move-result-object v6

    const-class v3, Lcom/flyme/deviceoriginalsettings/SubSettings;

    invoke-virtual {v3}, Ljava/lang/Class;->getName()Ljava/lang/String;

    move-result-object v7

    move v3, v1

    invoke-static/range {v0 .. v7}, Lcom/lenovo/settings/recentsettings/RecentSettingsApi;->addChangValueAction(Landroid/content/Context;ILjava/lang/String;ILjava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V

    .line 260
    invoke-interface {p1}, Landroid/view/MenuItem;->getItemId()I

    move-result v0

    packed-switch v0, :pswitch_data_0

    .line 309
    invoke-super {p0, p1}, Lcom/flyme/deviceoriginalsettings/bluetooth/DeviceListPreferenceFragment;->onOptionsItemSelected(Landroid/view/MenuItem;)Z

    move-result v0

    :goto_0
    return v0

    .line 262
    :pswitch_0
    iget-object v0, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mLocalAdapter:Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;

    invoke-virtual {v0}, Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;->getBluetoothState()I

    move-result v0

    const/16 v1, 0xc

    if-ne v0, v1, :cond_0

    .line 266
    iget-object v0, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mLocalAdapter:Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;

    invoke-virtual {v0}, Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;->isDiscovering()Z

    move-result v0

    if-eqz v0, :cond_1

    .line 268
    iget-object v0, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mLocalAdapter:Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;

    invoke-virtual {v0}, Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;->stopScanning()V

    :cond_0
    :goto_1
    move v0, v11

    .line 275
    goto :goto_0

    .line 271
    :cond_1
    invoke-direct {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->startScanning()V

    goto :goto_1

    .line 279
    :pswitch_1
    const-string v0, "BluetoothSettings"

    const-string v1, "display bluetoothName dialog"

    invoke-static {v0, v1}, Landroid/util/Log;->d(Ljava/lang/String;Ljava/lang/String;)I

    .line 281
    new-instance v0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothNameDialogFragment;

    invoke-direct {v0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothNameDialogFragment;-><init>()V

    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getFragmentManager()Landroid/app/FragmentManager;

    move-result-object v1

    const-string v2, "rename device"

    invoke-virtual {v0, v1, v2}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothNameDialogFragment;->show(Landroid/app/FragmentManager;Ljava/lang/String;)V

    move v0, v11

    .line 283
    goto :goto_0

    .line 286
    :pswitch_2
    new-instance v0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothVisibilityTimeoutFragment;

    invoke-direct {v0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothVisibilityTimeoutFragment;-><init>()V

    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getFragmentManager()Landroid/app/FragmentManager;

    move-result-object v1

    const-string v2, "visibility timeout"

    invoke-virtual {v0, v1, v2}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothVisibilityTimeoutFragment;->show(Landroid/app/FragmentManager;Ljava/lang/String;)V

    move v0, v11

    .line 288
    goto :goto_0

    .line 291
    :pswitch_3
    new-instance v10, Landroid/content/Intent;

    const-string v0, "android.btopp.intent.action.OPEN_RECEIVED_FILES"

    invoke-direct {v10, v0}, Landroid/content/Intent;-><init>(Ljava/lang/String;)V

    .line 292
    .local v10, "intent":Landroid/content/Intent;
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getActivity()Landroid/app/Activity;

    move-result-object v0

    invoke-virtual {v0, v10}, Landroid/app/Activity;->sendBroadcast(Landroid/content/Intent;)V

    move v0, v11

    .line 293
    goto :goto_0

    .line 297
    .end local v10    # "intent":Landroid/content/Intent;
    :pswitch_4
    new-instance v9, Landroid/content/Intent;

    const-string v0, "android.intent.action.MAIN"

    invoke-direct {v9, v0}, Landroid/content/Intent;-><init>(Ljava/lang/String;)V

    .line 298
    .local v9, "i":Landroid/content/Intent;
    const-string v0, "com.mediatek.bluetooth.settings.action.START_BT_ADV_SETTING"

    invoke-virtual {v9, v0}, Landroid/content/Intent;->setAction(Ljava/lang/String;)Landroid/content/Intent;

    .line 300
    :try_start_0
    invoke-virtual {p0, v9}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->startActivity(Landroid/content/Intent;)V
    :try_end_0
    .catch Landroid/content/ActivityNotFoundException; {:try_start_0 .. :try_end_0} :catch_0

    move v0, v11

    .line 305
    goto :goto_0

    .line 301
    :catch_0
    move-exception v8

    .line 302
    .local v8, "e":Landroid/content/ActivityNotFoundException;
    const-string v0, "BluetoothSettings"

    new-instance v1, Ljava/lang/StringBuilder;

    invoke-direct {v1}, Ljava/lang/StringBuilder;-><init>()V

    const-string v2, "Unable to start activity "

    invoke-virtual {v1, v2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v1

    invoke-virtual {v9}, Landroid/content/Intent;->toString()Ljava/lang/String;

    move-result-object v2

    invoke-virtual {v1, v2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v1

    invoke-virtual {v1}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v1

    invoke-static {v0, v1}, Landroid/util/Log;->e(Ljava/lang/String;Ljava/lang/String;)I

    .line 303
    const/4 v0, 0x0

    goto/16 :goto_0

    .line 260
    :pswitch_data_0
    .packed-switch 0x1
        :pswitch_0
        :pswitch_1
        :pswitch_2
        :pswitch_3
        :pswitch_4
    .end packed-switch
.end method

.method public onPause()V
    .locals 2

    .prologue
    .line 201
    invoke-super {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/DeviceListPreferenceFragment;->onPause()V

    .line 202
    iget-object v0, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mBluetoothEnabler:Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothEnabler;

    if-eqz v0, :cond_0

    .line 203
    iget-object v0, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mBluetoothEnabler:Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothEnabler;

    invoke-virtual {v0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothEnabler;->pause()V

    .line 205
    :cond_0
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getActivity()Landroid/app/Activity;

    move-result-object v0

    iget-object v1, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mReceiver:Landroid/content/BroadcastReceiver;

    invoke-virtual {v0, v1}, Landroid/app/Activity;->unregisterReceiver(Landroid/content/BroadcastReceiver;)V

    .line 206
    iget-object v0, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mDiscoverableEnabler:Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDiscoverableEnabler;

    if-eqz v0, :cond_1

    .line 207
    iget-object v0, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mDiscoverableEnabler:Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDiscoverableEnabler;

    invoke-virtual {v0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDiscoverableEnabler;->pause()V

    .line 209
    :cond_1
    return-void
.end method

.method public onResume()V
    .locals 3

    .prologue
    .line 176
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getActivity()Landroid/app/Activity;

    move-result-object v0

    invoke-static {v0}, Lcom/lenovo/settings/LenovoUtils;->setContext(Landroid/content/Context;)V

    .line 181
    iget-object v0, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mBluetoothEnabler:Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothEnabler;

    if-eqz v0, :cond_0

    .line 182
    iget-object v0, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mBluetoothEnabler:Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothEnabler;

    invoke-virtual {v0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothEnabler;->resume()V

    .line 184
    :cond_0
    invoke-super {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/DeviceListPreferenceFragment;->onResume()V

    .line 186
    iget-object v0, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mDiscoverableEnabler:Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDiscoverableEnabler;

    if-eqz v0, :cond_1

    .line 187
    iget-object v0, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mDiscoverableEnabler:Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDiscoverableEnabler;

    invoke-virtual {v0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDiscoverableEnabler;->resume()V

    .line 190
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getActivity()Landroid/app/Activity;

    move-result-object v0

    invoke-static {v0}, Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothManager;->getInstance(Landroid/content/Context;)Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothManager;

    move-result-object v0

    iget-object v1, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mDiscoverableEnabler:Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDiscoverableEnabler;

    invoke-virtual {v0, v1}, Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothManager;->setDiscoverableEnabler(Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothDiscoverableEnabler;)V

    .line 193
    :cond_1
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getActivity()Landroid/app/Activity;

    move-result-object v0

    iget-object v1, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mReceiver:Landroid/content/BroadcastReceiver;

    iget-object v2, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mIntentFilter:Landroid/content/IntentFilter;

    invoke-virtual {v0, v1, v2}, Landroid/app/Activity;->registerReceiver(Landroid/content/BroadcastReceiver;Landroid/content/IntentFilter;)Landroid/content/Intent;

    .line 194
    iget-object v0, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mLocalAdapter:Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;

    if-eqz v0, :cond_2

    .line 195
    iget-object v0, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mLocalAdapter:Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;

    invoke-virtual {v0}, Lcom/flyme/deviceoriginalsettings/bluetooth/LocalBluetoothAdapter;->getBluetoothState()I

    move-result v0

    iget-boolean v1, p0, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->mActivityStarted:Z

    invoke-direct {p0, v0, v1}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->updateContent(IZ)V

    .line 197
    :cond_2
    return-void
.end method

.method public onScanningStateChanged(Z)V
    .locals 1
    .param p1, "started"    # Z

    .prologue
    .line 448
    invoke-super {p0, p1}, Lcom/flyme/deviceoriginalsettings/bluetooth/DeviceListPreferenceFragment;->onScanningStateChanged(Z)V

    .line 450
    invoke-virtual {p0}, Lcom/flyme/deviceoriginalsettings/bluetooth/BluetoothSettings;->getActivity()Landroid/app/Activity;

    move-result-object v0

    invoke-virtual {v0}, Landroid/app/Activity;->invalidateOptionsMenu()V

    .line 451
    return-void
.end method
